...
export class SequencerEngine {
    constructor() {
        ...
        this.globalBars = 4; // Default length

        // Data Model (The 4 Groups x 12 Pads)
        // We initialize this empty, UI can populate it or we default it here
        ...
        // Callbacks (Hooks for the UI to listen to)
        this.onStepTrigger = null; // func(groupIndex, padIndex, velocity)
        this.onClockTick = null;   // func(stepIndex)
        this.onLog = null;         // func(message)
    }
    ...
    async init() {
        try {
            ...
            // Auto-select Teenage Engineering devices if present
    ...
    scheduleNote(beatNumber, time) {
        // 1. MIDI Clock Pulses (24ppq - approximation)
        // Purely logic side, we send the raw bytes
        if (this.midiOut) {
            const secondsPer16th = (60.0 / this.bpm) * 0.25;
            const pulseInterval = secondsPer16th / 6;
            for (let i = 0; i < 6; i++) {
                // We don't want to flood the queue too far ahead, but for WebAudio/MIDI this is standard
                this.midiOut.send([0xF8], performance.now() + ((time + (i * pulseInterval)) - this.audioCtx.currentTime) * 1000);
            }
        }

        // 2. Swing & Humanize Math
        ...
        // 3. Trigger Data
        // Iterate over all 4 groups (A, B, C, D)
        for (let g = 0; g < 4; g++) {
            const chan = g; // MIDI Ch 1-4
            ...
            for (let p = 0; p < 12; p++) {
                ...
                if (this.onStepTrigger) {
                    // We calculate visual delay so UI flashes exactly when audio hits
        ...
        // Notify UI of global clock step (for moving grid cursor)
        if (this.onClockTick) {
             const delayMs = (exactTime - this.audioCtx.currentTime) * 1000;
    ...
    sendChord(pad, chan, baseVel, timestamp) {
        ...
        intervals.forEach((interval, i) => {
            ...
            const strum = i * (5 + (fluxVal * 20)); // Strum effect
            // Randomize velocity slightly based on flux
